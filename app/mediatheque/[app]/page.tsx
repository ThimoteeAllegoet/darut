...
import { ApplicationName, MediathequeItem } from '../../types/mediatheque';
...
export default function MediathequeAppPage() {
  const router = useRouter();
  const params = useParams();
  const { isAuthenticated } = useAuth();
  const { items, addItem, updateItem, deleteItem, getItemsByApp } = useMediatheque();
  ...
  const [editingItem, setEditingItem] = useState<MediathequeItem | null>(null);
  ...
  const handleEditItem = (item: MediathequeItem) => {
    setEditingItem(item);
    setTitle(item.title);
    setUrl(item.url);
    setIsModalOpen(true);
